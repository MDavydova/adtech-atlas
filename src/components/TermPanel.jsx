  ...
  return (
    <div className="fixed bottom-5 left-1/2 -translate-x-1/2 w-[calc(100%-40px)] max-w-[700px] bg-white border border-[#c8b89a] rounded-xl shadow-2xl z-50 overflow-hidden">
      ...
      <div className="flex justify-between items-start gap-4 px-7 pt-6 pb-4 border-b border-[#e2d8cc]">
        <h3 className="font-serif font-normal text-[22px] text-[#a87c28] leading-tight">
          {term.name}
        </h3>
        <button
          onClick={onClose}
      ...
      {/* Scrollable body */}
      <div className="overflow-y-auto max-h-[60vh] px-7 py-5 space-y-5">
        {/* Short definition — always shown */}
        <p className="text-[15px] text-[#6b5b4e] leading-relaxed">{term.def}</p>

        {/* Article content — shown only when it exists */}
        {a && (
          <div className="space-y-4 border-t border-[#e2d8cc] pt-4">
            {a.summary && (
              <div>
                <p className="text-[11px] font-semibold tracking-widest uppercase text-[#a87c28] mb-1">
                  The full picture
                </p>
                <p className="text-[14px] text-[#6b5b4e] leading-relaxed">
                  {a.summary}
                </p>
              </div>
            )}
        ...
        {relatedTerms.length > 0 && (
          <div className="border-t border-[#e2d8cc] pt-4 text-[13px] text-[#a8978a]">
            <span className="text-[10px] font-semibold tracking-widest uppercase text-[#a87c28] mr-2">
              connects to
            </span>
